...
public class SlidingWindowCollection<T> implements Iterable<TimeWindow<T>> {
    ...
    /**
     * Instantiates a new SlidingWindowCollection which performs evictions on entries that arrive before the parameter startTimeMs.
     *
     ...
     */
    public SlidingWindowCollection(List<T> sourceCollection, TimestampSelector<T> timestampSelector, long windowDurationMs, long everyMs, long startTimeMs) {
        init(sourceCollection, timestampSelector, windowDurationMs, everyMs, startTimeMs, new DefaultWatermarkGenerator(startTimeMs));
    }
     ...
     * @param windowDurationMs the duration of a time window
     * @param everyMs the time between the starting point of each time window
     * @param startTimeMs the first time an object can be in a time window
     * @param watermarkGenerator used to generate a watermark. Entries that arrive before the watermark time are evicted.
     */
    public SlidingWindowCollection(List<T> sourceCollection, TimestampSelector<T> timestampSelector, long windowDurationMs, long everyMs, long startTimeMs, WatermarkGenerator watermarkGenerator) {
        init(sourceCollection, timestampSelector, windowDurationMs, everyMs, startTimeMs, watermarkGenerator);
    }

    private void init(List<T> sourceCollection, TimestampSelector<T> timestampSelector, long windowDurationMs, long everyMs, long startTimeMs, WatermarkGenerator watermarkGenerator) {
        ...
        if(startTimeMs < 0) throw new IllegalArgumentException("startTimeMs <= 0 in param");
